...
const USER_DEFAULTS = {
    mode: 'URL Link',
    url: 'https://linktr.ee/MS_Digimark?utm_source=linktree_profile_share&ltsid=3ac17aa9-833b-4e8f-aa3b-16d1a28bf0fa',
    vName: 'Sabo Sugi',
    vEmail: 'sugirov@gmail.com',
    vPhone1: '+7 708 9635525',
    vPhone2: '',
    vWeb: 'https://linktr.ee/MS_Digimark?utm_source=linktree_profile_share&ltsid=3ac17aa9-833b-4e8f-aa3b-16d1a28bf0fa',
    cutScale: 0.450,
    border: false
};

const qrMount = document.querySelector('.qr-cube-canvas');
if (!qrMount) {
    throw new Error('QR cube mount not found');
}

const getWidth = () => qrMount.clientWidth || 160;
const getHeight = () => qrMount.clientHeight || 160;

// --- 1. Scene Setup ---
const scene = new THREE.Scene();
scene.background = null;

const camera = new THREE.PerspectiveCamera(18, getWidth() / getHeight(), 10, 1000);
...
qrMount.appendChild(renderer.domElement);
...
function generateQR() {
    ...
    qrGroup.scale.setScalar(0.74);
    qrGroup.rotation.x = -0.45;
    qrGroup.rotation.y = 0.6;
}
...
new ResizeObserver(resizeRenderer).observe(qrMount);
